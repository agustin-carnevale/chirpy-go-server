...
const downgradeUser = `-- name: DowngradeUser :execrows
UPDATE users
SET is_chirpy_red = FALSE
WHERE id = $1
`

func (q *Queries) DowngradeUser(ctx context.Context, id uuid.UUID) (int64, error) {
	result, err := q.db.ExecContext(ctx, downgradeUser, id)
	if err != nil {
		return 0, err
	}
